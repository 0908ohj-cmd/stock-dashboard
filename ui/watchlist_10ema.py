...
def render_10ema_tab(market: str, label: str):
    ...
    with st.expander('셋업 기준 & 컬럼 설명', expanded=False):
        c1, c2 = st.columns(2)
        c1.markdown(
            '**🟢 셋업** — 브레이크아웃 직전  \n'
            '타이트 횡보 3~40일 · 베이스 범위 ≤15% · 거래량 수축 · 10EMA 서핑  \n'
            '→ 타점(기준봉 고가) 돌파 시 ORH 매수\n\n'
            '**🟡 형성중** — 기준봉 있으나 조건 미충족  \n'
        ...
        c2.markdown(
            ...
            '**⚫ 이탈** — 기준봉 저가 하방  \n\n'
            '**⚫ 없음** — 최근 3개월(63거래일) 내 기준봉 미탐지'
        )
        st.divider()
        st.markdown(
            '| 컬럼 | 설명 | 포인트 |\n'
            '|------|------|--------|\n'
            '| 타점 | 기준봉 고가 — 이 가격 돌파 시 ORH 매수 | 진입가 |\n'
            '| 현재→타점% | 현재가 대비 타점까지 거리 (음수 = 아직 아래) | **−5% 이내** 주목 |\n'
            '| 손절 | 기준봉 (고+저)/2 — 이탈 시 즉시 손절 | 중간선 |\n'
            '| 리스크% | (타점−손절)/타점 — 진입 시 최대 손실 | 작을수록 유리 |\n'
            '| 이전상승% | 기준봉 전 3개월 저점→고가 상승폭 (Prior Move) | **30%+** 필수 |\n'
            '| 횡보일수 | 기준봉 이후 경과 거래일 | 3~40일 |\n'
            '| 기준봉거래량비 | 기준봉 거래량 / 직전 20일 평균 | 클수록 강한 피벗 |\n'
            '| MA점수 | EMA10/21 · SMA50/150/200 위 개수 (0~5) | 5 = 완전 정배열 |\n'
            '| 고점대비% | 52주 고점 대비 낙폭 | −30% 이내 권장 |'
        )
        st.divider()
