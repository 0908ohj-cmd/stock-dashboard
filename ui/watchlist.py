...
def render_watchlist_tab(tickers: list, market: str, label: str):
    ...
    with st.expander('사용 가이드', expanded=False):
        ...
        st.divider()
        st.caption('찐반등 감지 조건 (5가지 모두 충족 시)')
        st.markdown(
            '| # | 조건 | 비고 |\n'
            '|---|------|------|\n'
            '| 1 | 장중 **Low < EMA21** | 저가가 EMA21 아래로 눌린 자리에서 반등 |\n'
            '| 2 | 직전봉 종가 < 전전봉 종가 | 찐반등 전날이 그 전날보다 낮게 마감 → 하락 추세 중 반등 |\n'
            '| 3 | **양봉** (종가 ≥ 시가) | 강하게 반등 |\n'
            '| 4 | 상승폭 **≥ ADR(20일)** | 충분히 큰 움직임 |\n'
            '| 5 | 당일 바디 **≥ 직전봉 바디 × 50%** | 전날 음봉 절반 이상 커버 |'
        )
    ...
    if top_candidates or fallback:
        ...
        ref_date    = jjin_date_str if (state == 'early_signal' and jjin_date_str) else (jjin_date_str or '진행 중')
