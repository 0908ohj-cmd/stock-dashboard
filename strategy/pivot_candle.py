...
def find_pivot_candle(
    ...
    안에서는 첫 봉이 낮은 진입가를 준다):
      1) 저가가 이후 종가로 뚫린 후보를 먼저 버린다
      2) 남은 후보를 클러스터로 묶고 각 클러스터의 첫 봉만 남긴다.
         간격은 '직전 후보'가 아니라 '클러스터 첫 봉'에서 재므로, 78/85/92처럼
         7거래일씩 연쇄하면 {78}/{92}로 갈린다 (의도 미확정 — 아래 주석 참고)
      3) 클러스터가 여럿이면 가장 최근 클러스터를 쓴다
    """
    if len(stock_df) < 70:
        return None
    ...
    # 클러스터 첫 봉에서 10거래일 이내면 같은 클러스터로 보고 첫 봉 우선,
    # 클러스터 간(>10일)에는 가장 최근 클러스터 선택.
    # ⚠️ 간격 기준이 '직전 후보'가 아니라 '클러스터 첫 봉'이라, 후보가 7거래일씩
    #    연쇄하면(78/85/92) 한 흐름인데도 둘로 갈려 뒤 봉이 선택된다.
    #    또 무효화가 이 앞에서 일어나므로, 버려진 후보 유무가 살아남은 후보 간
    #    선택을 뒤집을 수 있다(70 유효 시 85 / 70 무효 시 78).
    #    둘 다 의도 확인 전이라 현행 유지 — 동작은 테스트로 고정해 뒀다.
    clustered = [valid[0]]
