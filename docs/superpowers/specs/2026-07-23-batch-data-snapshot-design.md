...
1. **파일 업로드 직후**: 해당 시장 전체를 1회 수집·분석해 즉시 표시. 이 스냅샷을 다음 장 마감 배치까지 사용
2. **장 마감 후 배치**: 해당일 OHLCV까지 다시 받아 갱신 — KR(KOSPI·KOSDAQ) 매일 16:00 KST, US(NASDAQ) 매일 07:00 KST
3. 갱신된 스냅샷을 **다음날 배치까지** 그대로 사용 (그 사이 로딩 없음)
...
- 파일 업로드 직후에는 해당 시장을 즉시 1회 수집·분석해 바로 보여줌
...
- **최종 와치리스트(분석 결과 rows) 저장 — 하지 않음.** 저장 대상은 OHLCV 스냅샷(재료)이고, 리스트는 그 위에서 즉석 계산(수 초). Swing Low 날짜·커스텀 RS 기산점 등 사용자가 UI에서 바꾸는 입력을 반영해야 하므로 결과를 고정 저장할 수 없다
...
scripts/fetch_snapshot.py                 app.py
  ├ data/saved/*.tickers 읽기              ├ data/store.py 로더로 JSON 읽기 (네트워크 0)
  ├ fetch_daily (pykrx/FDR 패치 체인)      ├ 업로드 직후엔 해당 시장 즉시 1회 수집
  ├ data/ohlcv/*.json 전체 교체            ├ JSON 내 메타로 신선도 배지 표시
...
```
data/ohlcv/
├── KR_KOSPI.json     # 시장별 스냅샷: 메타 + 종목별 OHLCV
├── KR_KOSDAQ.json
├── US.json
└── indices.json      # KOSPI·KOSDAQ·NASDAQ — 400일
```

- 시장별 JSON 스키마 (메타를 파일 안에 포함 — 별도 meta.json 없음):

```json
{
  "market": "KR_KOSPI",
  "fetched_at": "2026-07-27T16:00:12+09:00",
  "last_trading_date": "2026-07-27",
  "ticker_count": 87,
  "failed": ["123456"],
  "data": {
    "005930": {
...
- `indices.json`도 동일 구조 (`data` 키가 KOSPI/KOSDAQ/NASDAQ)
- 종목 시세는 350달력일(≈240거래일) — 현재 `fetch_daily(days=350)`와 동일. SMA200·52주 고점 계산 요건
- 매 배치 **전체 교체** (해당 시장 JSON을 통째로 다시 씀). 소스(yfinance `auto_adjust`, pykrx)가 보정한 값으로 전량 갱신되므로 분할·배당 보정 문제 없음
- 예상 크기: 시장당 수 MB (parquet보다 크지만 허용 범위, 추가 의존성 없음)
...
- 동작:
  ...
  4. JSON 전체 교체 (휴장일이어도 `fetched_at`은 항상 갱신)
- 실패 처리:
  - 개별 티커 실패 → 스킵하고 계속, `failed` 배열에 기록
  - 시장 단위 대량 실패(성공률 70% 미만) → 해당 시장 JSON을 **교체하지 않고** 종료 코드 1 (이전 데이터 유지, Actions 잡 실패로 표시)
- Streamlit 무의존 (data/ 계층만 import)

### `.github/workflows/fetch-data.yml`

- 트리거:
  - `schedule`: KR `0 7 * * 1-5` (UTC, = **16:00 KST** 월~금) → `--markets kr`
  - `schedule`: US `0 22 * * 1-5` (UTC, = **07:00 KST** 화~토, EDT 18:00/EST 17:00 마감 후) → `--markets us`
  - `workflow_dispatch`: 수동 실행 (markets 입력 가능) — 비상 복구용
- 잡 순서: checkout → python 3.11 → 의존성 설치(`requirements-batch.txt` — pandas·yfinance·pykrx·finance-datareader만, streamlit 제외) → 스크립트 실행 → `git pull --rebase` 후 `data/ohlcv/` 커밋·push
- 커밋 메시지: `data: KR 스냅샷 2026-07-27` 형식
...
## 7. 앱 읽기 경로 — 새 모듈 `data/store.py`

Streamlit 무의존 (캐시 래핑은 ui/ 계층에서).

- `load_daily(ticker, market) -> DataFrame`
  - 해당 시장 JSON에 티커가 있으면 즉시 반환 (fetch_daily와 동일한 형태: DatetimeIndex + OHLCV 컬럼)
  - **미스 시** (스냅샷에 없는 티커): 기존 `fetch_daily`로 수집 → 컨테이너 로컬 JSON에 병합 저장 후 반환 (안전망)
- `load_index(name) -> DataFrame` — indices.json에서 로드, 미스 시 `fetch_index_daily` 폴백
- `get_freshness(market) -> dict` — 시장 JSON의 메타 기반 `{fetched_at, last_trading_date, is_stale}` 반환
- `refetch_market(market)` — 해당 시장 전체를 fetch_daily로 재수집해 로컬 JSON 교체 (업로드 직후·수동 새로고침용)
...
2. **업로드 직후 해당 시장 `refetch_market` 1회 실행** (스피너 표시) → 컨테이너 로컬 JSON 저장 → 즉시 분석·표시. 이 스냅샷을 다음 배치까지 사용
3. 알려진 특성: `_github_save` 커밋이 재배포를 유발하면 컨테이너 로컬 수집분은 소실되나, 재배포 후 미스 폴백이 다시 채운다. 다음 배치부터는 repo 번들에 포함

## 8. 신선도 표시 + 실패 감지

- 각 추세추종 탭 상단 + 지수 패널에 항상 표시:
  - `📅 07-27 장마감 기준 · 수집 07-27 16:00`
- **실패 판정 = 시장 JSON `fetched_at`이 "직전 평일 배치 예정 시각(KR 16:00 KST / US 07:00 KST) + 6시간"보다 이전** (주말·월요일 오전에 마지막 금요일 배치가 오탐되지 않도록 직전 평일 기준으로 계산)
  - 거래일 비교 방식은 쓰지 않는다 — 설날·추석 등 휴장일 오탐 방지. 배치가 휴장일에도 `fetched_at`을 갱신하므로 수집시각만으로 "배치가 돌았는가"를 정확히 판정 가능
  - 단, 업로드 직후·수동 재수집으로 로컬 갱신된 경우 그 `fetched_at`이 우선 (더 최신이므로 자연히 신선 판정)
- 실패 감지 시: ⚠️ **"배치 수집이 실패한 것 같습니다 — 사이드바에서 수동 새로고침 하세요"** 경고 배지만 표시. **자동 재수집은 하지 않는다**
- 시장 JSON이 아예 없으면(마이그레이션 직후 등) 경고 없이 미스 폴백으로 동작
...
| 상황 | 동작 |
|------|------|
| 배치에서 개별 티커 실패 | 스킵, JSON `failed` 배열에 기록 |
| 배치 시장 단위 대량 실패 (성공률 <70%) | JSON 미교체, 잡 실패 (이전 데이터 유지) |
| 배치 자체 미실행 (신선도 판정 초과) | 앱에서 ⚠️ 경고 배지, 수동 새로고침 유도 |
| 앱에서 스냅샷에 티커 없음 | fetch_daily 온디맨드 폴백 + 로컬 병합 |
| 시장 JSON 파일 없음 | 전체 미스 폴백 (기존 동작과 동일), 경고 없음 |
| 온디맨드 폴백 실패 | 해당 티커 스킵 (기존 `_build_rows`의 예외 처리 유지) |

## 11. 테스트

- `tests/test_store.py`:
  - JSON 히트 시 fetch_daily 미호출 (모킹)
  - 미스 시 폴백 호출 + 로컬 병합 저장
  - `refetch_market` 전체 교체 동작
  - `get_freshness` 판정: 신선/초과/파일 없음/주말 케이스
...
3. ui 호출부 교체 (업로드 직후 수집 포함) + 신선도 배지 + 새로고침 버튼 변경
